...
const registerPaymentInDB = (payment) => __awaiter(void 0, void 0, void 0, function* () {
    const res = yield defaultPost_1.default("http://localhost:3001/software/api/payment/register-in-db", payment);
    return res;
});
const paymentNotification = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const [client] = yield index_1.default.query(`SELECT id FROM clients WHERE mpId = '${user_id}'`);
        // 1. traer datos de la compra
        const getPaymentDataCall = yield getPaymentData(data.id);
        // 2. traer datos de planes
        const [pricing] = yield index_1.default.query(`SELECT * FROM pricing`);
        // 3. encontrar el plan al que se anoto
        const filterPlans = pricing.filter((plan) => plan.id === getPaymentDataCall.additional_info.items[0].id)[0].time;
        // 4. Actualizar datos de subscripcion en el perfil con esos datos
        const today = new Date();
        const updatePayment = yield clients_1.updatePaymentData(user_id, 1, getPaymentDataCall.additional_info.items[0].id, getToday_1.dateFormated, addMonths_1.default(filterPlans, today));
        // eslint-disable-next-line no-console
        console.log("updatePayment", updatePayment);
        // 5. crear pago nuevo
        const registerPaymentInDBCall = yield registerPaymentInDB({
            paymentId: data.id,
            preferenceId: "",
            ...
            date: getToday_1.dateFormated,
            paymentExpireDate: addMonths_1.default(filterPlans, today),
        });
        // eslint-disable-next-line no-console
        console.log("registerPaymentInDBCall", registerPaymentInDBCall);
        const [payment] = yield index_1.default.query(`SELECT id FROM payments WHERE mpId = ${user_id}`);
        if (!payment.length) {
            // a.
